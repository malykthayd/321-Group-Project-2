...
namespace api.Controllers
{
    ...
    public class SmsGatewayController : ControllerBase
    {
        ...
        // POST api/gateway/test/sms - Test endpoint to simulate receiving SMS
        [HttpPost("test/sms")]
        public async Task<IActionResult> TestReceiveSms([FromBody] TestSmsRequest request)
        {
            try
            {
                _logger.LogInformation("[TEST] Simulating SMS from {From}: {Text}", request.From, request.Text);

                // Convert test request to inbound request
                var inboundRequest = new InboundSmsRequest
                {
                    From = request.From,
                    To = "+15551234567", // Mock gateway number
                    Text = request.Text,
                    ProviderMeta = new Dictionary<string, string> { { "test", "true" } }
                };

                // Process as if it were a real inbound SMS
                var result = await ReceiveSms(inboundRequest);
                return result;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[TEST] SMS test error");
                return StatusCode(500, new { message = "Test failed", error = ex.Message });
            }
        }

        // POST api/gateway/test/send - Test endpoint to send SMS
        [HttpPost("test/send")]
        public async Task<IActionResult> TestSendSms([FromBody] TestSmsRequest request)
        {
            try
            {
                _logger.LogInformation("[TEST] Sending SMS to {To}: {Text}", request.From, request.Text);

                await SendSms(request.From, request.Text);
                return Ok(new { message = "Test SMS sent successfully" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[TEST] Send SMS test error");
                return StatusCode(500, new { message = "Test send failed", error = ex.Message });
            }
        }
        ...
        // POST api/gateway/twilio/webhook - Twilio webhook for incoming SMS
        ...
        public async Task<IActionResult> TwilioWebhook()
        {
            try
            {
                ...
                _logger.LogInformation("[TwilioWebhook] Inbound SMS from {From} to {To}: {Body}", from, to, body);
                ...
                // Process as if it were a regular inbound SMS
                var result = await ReceiveSms(inboundRequest);
                return result;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[TwilioWebhook] Error processing webhook");
