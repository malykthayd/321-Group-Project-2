...
class AdminManager {
    ...
    initializeEventListeners() {
        ...
        document.addEventListener('shown.bs.tab', (event) => {
            ...
            if (tabTarget === '#admin-dashboard-content') {
                ...
                console.log('User Management tab clicked, calling loadUserManagement');
                this.loadUserManagement();
            } else if (tabTarget === '#admin-accessibility-content') {
                // Initialize accessibility module
                console.log('Admin tab clicked: SMS/USSD');
                if (window.adminAccessibility) {
                    console.log('AdminAccessibility found, initializing...');
                    window.adminAccessibility.init();
                } else {
                    console.error('AdminAccessibility module not found');
                    // Try to initialize it
                    setTimeout(() => {
                        if (window.adminAccessibility) {
                            console.log('AdminAccessibility found on retry, initializing...');
                            window.adminAccessibility.init();
                        } else {
    ...
    async loadUserManagement() {
        ...
        if (!this.currentAdmin) {
            console.error('No currentAdmin found');
            return;
        }

        try {
            console.log('Fetching from:', `${this.apiBaseUrl}/admin/${this.currentAdmin.adminId}/user-management`);
    ...
    // AI Lesson Generator Methods
    async showAILessonGenerator() {
        console.log('showAILessonGenerator called');
        ...
        // Reset form
        document.querySelectorAll('#createLessonModal input[type="checkbox"]').forEach(cb => cb.checked = false);
        document.getElementById('startGenerationBtn').style.display = 'block';
        document.getElementById('publishAllBtn').style.display = 'none';
        document.getElementById('lessonGenStep1').style.display = 'block';
        document.getElementById('lessonGenStep2').style.display = 'none';
        document.getElementById('lessonGenStep3').style.display = 'none';
    }

    async generateAILessons() {
        const gradeCheckboxes = document.querySelectorAll('#lessonGenStep1 .col-md-6:first-child input[type="checkbox"]');
        const selectedGrades = Array.from(gradeCheckboxes).filter(cb => cb.checked).map(cb => cb.value);
        
        const subjectCheckboxes = document.querySelectorAll('#lessonGenStep1 .col-md-6:last-child input[type="checkbox"]');
        ...
        if (selectedGrades.length === 0 || selectedSubjects.length === 0) {
            alert('Please select at least one grade level and one subject');
            return;
        }

        // Show progress screen
        document.getElementById('lessonGenStep1').style.display = 'none';
        document.getElementById('lessonGenStep2').style.display = 'block';
        document.getElementById('startGenerationBtn').style.display = 'none';
        
        const progressDiv = document.getElementById('generationProgress');
        const requests = [];
        
        // Build requests for all grade/subject combinations
        for (const grade of selectedGrades) {
            for (const subject of selectedSubjects) {
                requests.push({
                    grade,
                    subject,
                    topic: this.generateTopicForSubject(subject, grade),
                    difficulty,
                    language: 'en'
                });
            }
        ...
        progressDiv.innerHTML = `<p>Generating ${requests.length} lessons...</p>`;

        try {
            // Call bulk generation API
            const response = await fetch(`${this.apiBaseUrl}/AILesson/generate-bulk`, {
                method: 'POST',
            ...
            if (response.ok && data.success) {
                this.displayGeneratedLessons(data.results, requests);
            } else {
                alert('Error generating lessons: ' + (data.error || 'Unknown error'));
                this.resetGeneratorUI();
            }
        } catch (error) {
            console.error('Error:', error);
            alert('Error generating lessons. Please try again.');
            this.resetGeneratorUI();
        }
    }

    generateTopicForSubject(subject, grade) {
        const topics = {
            'Math': ['Basic Addition', 'Subtraction Skills', 'Multiplication Tables', 'Division Concepts', 'Fractions'],
            'Science': ['Living Things', 'Weather Patterns', 'States of Matter', 'Plant Life', 'Animal Habitats'],
            'English': ['Reading Basics', 'Sentence Structure', 'Vocabulary Building', 'Story Elements', 'Grammar'],
            'Technology': ['Computer Basics', 'Internet Safety', 'Coding Fundamentals', 'Digital Citizenship', 'Keyboarding'],
            'Geography': ['World Continents', 'Countries and Capitals', 'Maps and Globes', 'Climate Zones', 'Natural Resources']
        };
        const options = topics[subject] || ['Introduction'];
        return options[Math.floor(Math.random() * options.length)];
    }

    displayGeneratedLessons(results, requests) {
        document.getElementById('lessonGenStep2').style.display = 'none';
        document.getElementById('lessonGenStep3').style.display = 'block';
        document.getElementById('startGenerationBtn').style.display = 'none';
        document.getElementById('publishAllBtn').style.display = 'block';

        const container = document.getElementById('generatedLessonsContainer');
        container.innerHTML = '';

        results.forEach((result, index) => {
            const request = requests[index];
            const card = this.createLessonCard(result, request, index);
            container.appendChild(card);
        });
    }

    createLessonCard(result, request, index) {
        ...
        card.innerHTML = `
            <div class="card h-100" id="lessonCard${index}">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div>
                        <strong>${request.subject} - ${request.grade}</strong>
                        <br><small class="text-muted">${lesson.title || request.topic}</small>
                    </div>
                ...
                <div class="card-body">
                    ${result.success ? `
                        ...
                        <p><small class="text-muted">Tokens used: ${result.tokens}</small></p>
                        <div class="btn-group w-100 mt-2">
                            <button class="btn btn-sm btn-outline-primary" onclick="window.adminManager.previewLesson(${index}, ${JSON.stringify(result).replace(/"/g, '&quot;')})">
                                <i class="bi bi-eye"></i> Preview
                            </button>
                            <button class="btn btn-sm btn-outline-success" onclick="window.adminManager.publishLesson(${index}, ${JSON.stringify(result).replace(/"/g, '&quot;')}, ${JSON.stringify(request).replace(/"/g, '&quot;')})">
                                <i class="bi bi-check-circle"></i> Publish
                            </button>
                            <button class="btn btn-sm btn-outline-danger" onclick="window.adminManager.deleteGeneratedLesson(${index})">
                                <i class="bi bi-trash"></i> Delete
                            </button>
                        </div>
                    ` : `
                        <p class="text-danger">${result.error || 'Generation failed'}</p>
                        <button class="btn btn-sm btn-outline-secondary w-100" onclick="window.adminManager.deleteGeneratedLesson(${index})">
    ...
    previewLesson(index, result) {
        ...
        if (sections.prior) {
            previewHTML += `<div class="mb-3"><strong>Prior Knowledge:</strong> ${sections.prior.question || 'N/A'}</div>`;
        }
        
        // Direct instruction
        if (sections.direct) {
            previewHTML += `<div class="mb-3"><strong>Direct Instruction:</strong><br>${sections.direct.content || 'N/A'}</div>`;
        }
        
        // Guided practice
        if (sections.guided && Array.isArray(sections.guided)) {
            previewHTML += `<div class="mb-3"><strong>Guided Practice Questions:</strong><ol>`;
            sections.guided.forEach((q, i) => {
                previewHTML += `<li>${q.q || q.question || `Question ${i + 1}`}</li>`;
            });
            previewHTML += `</ol></div>`;
        }
        
        // Independent practice
        if (sections.independent && Array.isArray(sections.independent)) {
            previewHTML += `<div class="mb-3"><strong>Independent Practice:</strong><ol>`;
            sections.independent.forEach((ex, i) => {
                previewHTML += `<li>${ex.description || ex.q || `Exercise ${i + 1}`}</li>`;
            });
            previewHTML += `</ol></div>`;
        }
        
        // Mastery check
        if (sections.mastery && Array.isArray(sections.mastery)) {
            previewHTML += `<div class="mb-3"><strong>Mastery Check Questions:</strong><ol>`;
            sections.mastery.forEach((q, i) => {
                previewHTML += `<li>${q.q || q.question || `Question ${i + 1}`}</li>`;
            });
            previewHTML += `</ol></div>`;
        }
        ...
        // Vocabulary
        if (sections.vocabulary && Array.isArray(sections.vocabulary)) {
            previewHTML += `<div class="mb-3"><strong>Vocabulary:</strong><ul>`;
            sections.vocabulary.forEach(vocab => {
                previewHTML += `<li><strong>${vocab.term || ''}:</strong> ${vocab.definition || ''}</li>`;
            });
            previewHTML += `</ul></div>`;
        }
        ...
        const modalHTML = `
            <div class="modal fade" id="lessonPreviewModal${index}">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Lesson Preview - Complete Content</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            ${previewHTML}
                            ...
                            <details class="mt-3">
                                ...
                                <pre class="mt-2 bg-light p-2" style="max-height: 300px; overflow-y: auto;">${JSON.stringify(lessonData, null, 2)}</pre>
                            </details>
                        </div>
                        <div class="modal-footer">
        ...
        const existingModal = document.getElementById(`lessonPreviewModal${index}`);
        if (existingModal) {
            existingModal.remove();
        }
        ...
        // Show modal
        const modal = new bootstrap.Modal(document.getElementById(`lessonPreviewModal${index}`));
        modal.show();
    ...
    async publishLesson(index, result, request) {
        try {
            const lessonData = JSON.parse(result.content);
            const lesson = lessonData.lesson || {};
            const sections = lessonData.sections || {};
            ...
            const instructionContent = sections.direct?.content || '';
            const guidedPractice = sections.guided || [];
            const independentPractice = sections.independent || [];
            const masteryCheck = sections.mastery || [];

            // Build HTML content
            let htmlContent = `<h3>${lesson.title || request.topic}</h3>`;
            htmlContent += `<p><strong>Objective:</strong> ${lesson.objective || ''}</p>`;
            htmlContent += `<h4>Introduction</h4>${instructionContent}`;
            
            if (guidedPractice.length > 0) {
                htmlContent += '<h4>Guided Practice</h4><ul>';
                guidedPractice.forEach(q => {
                    htmlContent += `<li>${q.q || q.question || ''}</li>`;
                });
                htmlContent += '</ul>';
            }

            if (independentPractice.length > 0) {
                htmlContent += '<h4>Independent Practice</h4><ul>';
                independentPractice.forEach(ex => {
                    htmlContent += `<li>${ex.description || ex.q || ''}</li>`;
                });
                htmlContent += '</ul>';
            }

            if (masteryCheck.length > 0) {
                htmlContent += '<h4>Mastery Check</h4><ul>';
                masteryCheck.forEach(q => {
                    htmlContent += `<li>${q.q || q.question || ''}</li>`;
                });
                htmlContent += '</ul>';
            }

            // Create lesson via admin API
            const response = await fetch(`${this.apiBaseUrl}/admin/${this.currentAdmin.adminId}/create-lesson`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    title: lesson.title || request.topic,
                    description: lesson.objective || '',
                    subject: request.subject,
                    gradeLevel: request.grade,
                    content: htmlContent,
                    resourceUrl: null,
                    tags: `${request.subject},${request.grade},AI Generated`,
                    isAvailable: true
                })
            });
            ...
            if (response.ok) {
                ...
                const card = document.getElementById('lessonCard' + index);
                if (card) {
                    card.classList.add('border-success');
                    const btn = card.querySelector('button[onclick*="publishLesson"]');
                    if (btn) {
                        btn.disabled = true;
                        btn.innerHTML = '<i class="bi bi-check-circle-fill"></i> Published';
                    }
                }
                this.loadDigitalLibrary();
            } else {
                alert('Error publishing lesson: ' + (data.message || 'Unknown error'));
            }
        } catch (error) {
            console.error('Error publishing lesson:', error);
            alert('Error publishing lesson. Please try again.');
        }
    }

    deleteGeneratedLesson(index) {
        const card = document.getElementById('lessonCard' + index);
        if (card) {
            card.remove();
        }
    ...
    async publishAllLessons() {
        const cards = document.querySelectorAll('[id^="lessonCard"]');
        let successCount = 0;
        let failCount = 0;

        for (const card of cards) {
            const publishBtn = card.querySelector('button[onclick*="publishLesson"]');
            if (publishBtn && !publishBtn.disabled) {
                // Extract data from onclick attribute
                const onclick = publishBtn.getAttribute('onclick');
                const match = onclick.match(/publishLesson\((\d+),\s*({[^}]+}),\s*({[^}]+})\)/);
                if (match) {
                    try {
                        const index = parseInt(match[1]);
                        const result = JSON.parse(match[2]);
                        const request = JSON.parse(match[3]);
                        await this.publishLesson(index, result, request);
                        successCount++;
                    } catch (e) {
                        failCount++;
                    }
        ...
        alert(`Published ${successCount} lessons${failCount > 0 ? `, ${failCount} failed` : ''}`);
...
function showCreateLessonModal() {
    console.log('showCreateLessonModal called');
    console.log('window.adminManager:', window.adminManager);
    
    if (!window.adminManager) {
        console.log('Creating adminManager...');
        window.adminManager = new AdminManager();
    }
    
    if (window.adminManager) {
        console.log('Calling showAILessonGenerator');
        window.adminManager.showAILessonGenerator();
    } else {
        console.error('adminManager still not found!');
        alert('Error: Unable to initialize admin manager. Please refresh the page.');
    }
}
...
// AI Lesson Generator Helper Functions
function selectAllGrades() {
    document.querySelectorAll('#lessonGenStep1 input[id^="grade"]').forEach(cb => cb.checked = true);
}

function selectAllSubjects() {
    document.querySelectorAll('#lessonGenStep1 input[id^="subject"]').forEach(cb => cb.checked = true);
}

// Event listeners for AI lesson generator
document.addEventListener('DOMContentLoaded', () => {
    const startGenBtn = document.getElementById('startGenerationBtn');
    const publishAllBtn = document.getElementById('publishAllBtn');
    
    if (startGenBtn) {
        startGenBtn.addEventListener('click', async () => {
            if (window.adminManager) {
                await window.adminManager.generateAILessons();
            }
        });
    }
    ...
    if (publishAllBtn) {
        publishAllBtn.addEventListener('click', async () => {
            if (window.adminManager) {
                await window.adminManager.publishAllLessons();
            }
        });
    }
});