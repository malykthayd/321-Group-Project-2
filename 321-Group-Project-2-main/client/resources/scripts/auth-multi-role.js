...
class MultiRoleAuthManager {
    ...
    selectLoginRole(role) {
        ...
        // Enable form fields when role is selected
    ...
    selectRegisterRole(role) {
        ...
        this.enableRegistrationForm(role);
    }
    ...
    // Enable login form when role is selected
    enableLoginForm() {
    ...
    // Enable registration form when role is selected
    enableRegistrationForm(role) {
        const formId = `${role}-registration`;
        const form = document.getElementById(formId);
        
        if (form) {
            // Enable all input fields in the selected registration form
            const inputs = form.querySelectorAll('input, select, button');
            inputs.forEach(input => {
                input.disabled = false;
                // Update placeholders
                if (input.placeholder && input.placeholder.includes('Select a role first')) {
                    if (input.type === 'email') {
                ...
                if (input.tagName === 'SELECT' && input.querySelector('option[value=""]')) {
                    input.querySelector('option[value=""]').textContent = 'Select Grade Level';
                }
            });
        }
    }
    ...
    redirectToRoleDashboard() {
        if (this.currentUser) {
            ...
            // Update UI for logged in user immediately
            this.updateUserInterface();
            ...
            // Force UI refresh to ensure all elements are updated
            setTimeout(() => {
                this.updateUserInterface();
